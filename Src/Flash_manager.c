...
extern uint16_t default_settings[HOLDING_REG_COUNT];
extern uint16_t settings[HOLDING_REG_COUNT];
...
{	
	volatile uint16_t flash_set[HOLDING_REG_COUNT+1];
	//uint32_t* flash_set = pvPortMalloc( sizeof(uint32_t)*HOLDING_REG_COUNT+1 );	
	volatile uint32_t orig_crc = 0;
	volatile uint32_t actual_crc = 0;	
	
	
	for (int i=0; i<HOLDING_REG_COUNT+1; i++)
	{
		flash_set[i] = read_flash(PAGE_ADDR + i*8);	
	}
	
	orig_crc = flash_set[HOLDING_REG_COUNT];
	
	actual_crc = crc16( (uint8_t*) &flash_set[0], HOLDING_REG_COUNT*2 );
	
	if (orig_crc == actual_crc)
	{
		for (int i=0; i<HOLDING_REG_COUNT; i++) data_out[i] = flash_set[i];
...
{	
	volatile uint32_t flash_set[HOLDING_REG_COUNT+2];
	//uint32_t* flash_set = pvPortMalloc( sizeof(uint32_t)*HOLDING_REG_COUNT+1 );
	uint32_t crc = 0;
	...
	crc = crc16( (uint8_t*) &settings[0], HOLDING_REG_COUNT*2 );	
	
	flash_set[HOLDING_REG_COUNT] = crc;	
	
	return write_flash(PAGE, (uint32_t*) &flash_set[0], HOLDING_REG_COUNT + 1);	
}
